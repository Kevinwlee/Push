...
@implementation CCHVaultService

+ (CCHVaultService *)sharedService {
    static CCHVaultService *__shared = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __shared = [[CCHVaultService alloc] init];
    });
    return __shared;
}
...
- (void)getItem:(NSDictionary *)item completion:(vaultCompletionBlock)completionBlock {
    ...
    CCHVaultProxy *proxy = [self vaultProxy];
...
- (CCHVaultProxy *)vaultProxy {
    if (!_vaultProxy) {
        _vaultProxy = [CCHVaultProxy sharedProxy];
    }
    return _vaultProxy;
}
